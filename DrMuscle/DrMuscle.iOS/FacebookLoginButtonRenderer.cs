...
namespace DrMuscle.iOS
{
	public class FacebookLoginButtonRenderer : ViewRenderer<FacebookLoginButton, LoginButton>
	{
		protected override void OnElementChanged(ElementChangedEventArgs<FacebookLoginButton> e)
		{
			...
			control.Permissions = element.ReadPermissions;
			control.Delegate = new CustomLoginButtonDelegate(element);
			...
			if (element.LoggedIn)
			{
				...
				var t = new GraphRequest("me");
				
				t.Start((IGraphRequestConnecting connection, Foundation.NSObject result, Foundation.NSError error) =>
				{
					var graphObject = result as NSDictionary;
    ...
    class CustomLoginButtonDelegate : LoginButtonDelegate
    {
		FacebookLoginButton element;
		public CustomLoginButtonDelegate(FacebookLoginButton facebookLogin)
        {
        ...
        public override void DidComplete(LoginButton loginButton, LoginManagerLoginResult result, NSError error)
        {
				...
				var accessToken = AccessToken.CurrentAccessToken;
				var t = new GraphRequest("me", new NSDictionary("fields", "id, first_name, email, last_name, picture.width(1000).height(1000)"));
				t.Start((IGraphRequestConnecting connection, Foundation.NSObject result1, Foundation.NSError error1) =>
				{
					var graphObject = result1 as NSDictionary;
